...
for (i in 1:13){
  ...
  all.medians[i] <- median(bseas.all[,i])
}
bseas.all <- rbind(bseas.all, all.means, all.medians)
rownames(bseas.all) <- c("Baltimore", "Boston", "Chicago", "Cincinnati", "Cleveland",
                         ...
                         "St. Louis", "San Francisco", "Toledo", "Washington, D.C.", "mean", "median")
...
b.seas.list <- list(b.seas.Baltimore[,2],b.seas.Boston[,2],b.seas.Chicago[,2],
                    ...
                    b.seas.WashDC[,2],bseas.all[18,],rep(NA,13))
